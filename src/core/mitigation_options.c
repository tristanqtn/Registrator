...
int set_mitigation_policy(void) {
    ...
    if (registry_value_exists(HKEY_LOCAL_MACHINE, MITIGATION_OPTIONS_REGISTRY_PATH, MITIGATION_OPTIONS_VALUE_NAME)) {
        DWORD dataSize;
        BYTE* value = get_registry_key(HKEY_LOCAL_MACHINE, MITIGATION_OPTIONS_REGISTRY_PATH, MITIGATION_OPTIONS_VALUE_NAME, &dataSize);
        if (value) {
...
int unset_mitigation_policy(void) {
    ...
    if (registry_value_exists(HKEY_LOCAL_MACHINE, MITIGATION_OPTIONS_REGISTRY_PATH, MITIGATION_OPTIONS_VALUE_NAME)) {
        ...
        if (set_registry_key(
            ...
            MITIGATION_OPTIONS_REGISTRY_PATH,
            MITIGATION_OPTIONS_VALUE_NAME,
            MITIGATION_OPTIONS_VALUE_DEFAULT,
            sizeof(MITIGATION_OPTIONS_VALUE_DEFAULT),
            REG_BINARY
            ) != 0) {
            pretty_print(LOG_ERROR, "Failed to set registry value");
            return -1;
