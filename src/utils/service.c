...
static SC_HANDLE open_scm(DWORD access) {
    ...
    if (!hSCM) {
        pretty_print(LOG_ERROR, "Failed to open Service Control Manager: %lu", GetLastError());
    }
 ...
 * @return: Handle on success, NULL on failure
 */
static SC_HANDLE open_service(SC_HANDLE hSCM, const char* serviceName, DWORD access) {
    ...
    if (!hService) {
        pretty_print(LOG_ERROR, "Failed to open service '%s': %lu", serviceName, GetLastError());
    }
...
/**
 * Poll the service status until it leaves a pending state or the timeout elapses
 * @param hService:       Handle to the service
 * @param pendingState:   The transitional state to wait through (e.g., SERVICE_STOP_PENDING)
 * @param targetState:    The expected final state (e.g., SERVICE_STOPPED)
 * @return: 1 if target state reached, 0 on timeout or error
 */
static int wait_for_service_state(SC_HANDLE hService, DWORD pendingState, DWORD targetState) {
    SERVICE_STATUS status;
    DWORD elapsed = 0;

    while (elapsed < SERVICE_STATUS_TIMEOUT_MS) {
        if (!QueryServiceStatus(hService, &status)) {
            pretty_print(LOG_ERROR, "Failed to query service status: %lu", GetLastError());
            return 0;
        }
        ...
        if (status.dwCurrentState != pendingState) {
            pretty_print(LOG_WARNING, "Service reached unexpected state: %lu", status.dwCurrentState);
            return 0;
        }
 ...
 * @param startType:   Start type (SERVICE_AUTO_START, SERVICE_DEMAND_START, SERVICE_DISABLED)
...
int create_service(const char* serviceName, const char* displayName,
    ...
    if (!hService) {
        pretty_print(LOG_ERROR, "Failed to create service '%s': %lu", serviceName, GetLastError());
...
int start_service(const char* serviceName) {
    ...
    SC_HANDLE hService = open_service(hSCM, serviceName, SERVICE_START | SERVICE_QUERY_STATUS);
    if (!hService) {
        CloseServiceHandle(hSCM);
        return -1;
    }
    ...
    if (!StartServiceA(
        ...
        pretty_print(LOG_ERROR, "Failed to start service '%s': %lu", serviceName, GetLastError());
        CloseServiceHandle(hService);
        CloseServiceHandle(hSCM);
        return -1;
    }

    if (!wait_for_service_state(hService, SERVICE_START_PENDING, SERVICE_RUNNING)) {
        pretty_print(LOG_ERROR, "Service '%s' did not reach running state", serviceName);
        CloseServiceHandle(hService);
        CloseServiceHandle(hSCM);
        return -1;
    }
...
int stop_service(const char* serviceName) {
    ...
    SC_HANDLE hService = open_service(hSCM, serviceName, SERVICE_STOP | SERVICE_QUERY_STATUS);
    if (!hService) {
        CloseServiceHandle(hSCM);
        return -1;
    }
    ...
    if (!ControlService(
        ...
        pretty_print(LOG_ERROR, "Failed to send stop control to '%s': %lu", serviceName, GetLastError());
        CloseServiceHandle(hService);
        CloseServiceHandle(hSCM);
        return -1;
    }

    if (!wait_for_service_state(hService, SERVICE_STOP_PENDING, SERVICE_STOPPED)) {
        pretty_print(LOG_ERROR, "Service '%s' did not reach stopped state", serviceName);
        CloseServiceHandle(hService);
        CloseServiceHandle(hSCM);
        return -1;
    }
...
int delete_service(const char* serviceName) {
    ...
    if (!DeleteService(hService)) {
        pretty_print(LOG_ERROR, "Failed to delete service '%s': %lu", serviceName, GetLastError());
        CloseServiceHandle(hService);
        CloseServiceHandle(hSCM);
        return -1;
    }
 ...
 * @param newBinaryPath:  New path to the service executable, or NULL to leave unchanged
 * @param newStartType:   New start type, or SERVICE_NO_CHANGE to leave unchanged
 * @return: 0 on success, -1 on failure
 * Links:
...
int modify_service(const char* serviceName, const char* newDisplayName,
    ...
    if (!ChangeServiceConfigA(
        ...
        pretty_print(LOG_ERROR, "Failed to modify service '%s': %lu", serviceName, GetLastError());
        CloseServiceHandle(hService);
        CloseServiceHandle(hSCM);
        return -1;
    }
